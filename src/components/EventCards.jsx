...
const EventCards = ({ eventData }) => {
  const { thumbnail, name, category, date, location, entry_fee, id } =
    eventData;
  return (
    <div className="border-2 border-base-300 p-5 rounded-2xl space-y-2">
      <img
        className="w-full md:h-60 md:min-h-64 rounded-2xl mx-auto"
        src={thumbnail}
        alt="event-photo"
      />
      ...
      <Link to={`/eventDetails/${id}`}>
        <button className="btn bg-primary text-white rounded-2xl">
          View more
        </button>
      </Link>
    </div>
